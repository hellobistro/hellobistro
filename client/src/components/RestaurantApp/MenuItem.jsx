...
class MenuItem extends React.Component {
  ...
  render() {
    ...
    const render = {
      name: this.state.editName ? <div><input type="text" placeholder={this.props.data.name} onChange={(e) => { this.handleChange(e); }} onBlur={(e) => { this.handleSave(e, 'editName'); }} onSubmit={(e) => { this.handleSave(e, 'editPrice'); }} /><button onClick={() => this.toggleEdit('editName')}><span aria-labelledby="cancel" role="img">❌</span></button></div> : <h3 onClick={() => this.toggleEdit('editName')} >{this.props.data.name}</h3>,
      price: this.state.editPrice ? <div><input type="text" placeholder={this.props.data.price} onChange={(e) => { this.handleChange(e); }} onBlur={(e) => { this.handleSave(e, 'editPrice'); }} onSubmit={(e) => { this.handleSave(e, 'editPrice'); }} /><button onClick={() => this.toggleEdit('editPrice')}><span aria-labelledby="cancel" role="img">❌</span></button></div> : <p onClick={() => this.toggleEdit('editPrice')} >${this.props.data.price}</p>,
